...
date: 2019-12-19
author: biubiu
header-img: 'assets/img/pro9.png'
catalog: true
tags: C++ test
---
## 实验室学习总结报告
...
imshow("自选名字",inputimage);//图像显示
imwrite("图片自选文件名路径",inputimage);//图像写入文件
waitKey();
```
...
```
namedWindow("窗口名"，0);//参数0表示窗口大小可调整；使用参数WINDOW_AUTOSIZE窗口将会与载入图像大小一致
imshow("窗口名"，inputimage);
```
...
int main()
{
	...
	temImage=srcImage;
	imshow("原图",srcImage);//显示原图	resize(temImage,dstImage1,Size(temImage.cols/2,temImage.rows/2),0,0,INTER_LINEAR);//缩小一倍	resize(temImage,dstImage2,Size(temImage.cols*2,temImage.rows*2),0,0,INTER_LINEAR); //放大一倍
	imshow("缩小",dstImage1);
	imshow("放大",dstImage2);
	waitKey();
	return 0;
...
warpAffine(初始图像, 形成图像, 变换矩阵,size());//进行变换
```
...
```
detector->detect(图像变量，keypoint变量);
```
...
```
drawKeypoints(原图，keypoint变量，新图变量，Scalar(0,0,255),参数)
```
...
descriptor->compute ( 原图, keypoints变量, descriptors );
```

​		7)直接得到特征点和描述子(AKAZE)

```
detector->detectAndCompute(原图, Mat(), keypoints变量, descriptors);
```
